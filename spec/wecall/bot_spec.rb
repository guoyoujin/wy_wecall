...
RSpec.describe WyWecall::Client do
  ...
  context 'query_bot_list' do
          ...
          "list": [
            {
              ...
              "createTime": 1700386380000,
              "updateTime": 1700386535000
            }
          ],
          "total": 80,
          "page": 1,
    ...
    it 'should get bot list' do
      expect_any_instance_of(Faraday::Connection).to receive(:get).with('/open/api/wecall/v1/bot/getBotList', { page: page, pageSize: page_size, botName: bot_name, status: status }).and_return(status: 200, response_headers: response_headers, body: response_body) 
      expect(client.query_bot_list(page, page_size, bot_name, status)).to eq({
        status: 200,
